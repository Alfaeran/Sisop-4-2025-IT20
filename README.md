...
// tambahan: file biasa dari base_dir
    DIR *dp;
    struct dirent *de;
    dp = opendir(base_dir);
    if (dp == NULL) return -ENOENT;
    while ((de = readdir(dp)) != NULL) {
        if (strcmp(de->d_name, "activity.log") == 0) continue;
        if (strcmp(de->d_name, "relics") == 0) continue;
        if (strcmp(de->d_name, FILENAME) == 0) continue;
...
static int baymax_open(const char *path, struct fuse_file_info *fi) {
    if (strcmp(path + 1, FILENAME) == 0) return 0;
...
static int baymax_read(const char *path, char *buf, size_t size,
    ...
    if (strcmp(path + 1, FILENAME) == 0) {
        ...
        for (int i = 0; i < PART_COUNT && read_total < size; i++) {
            ...
            if (offset < current_offset + part_size) {
                size_t local_offset = offset > current_offset ? offset - current_offset : 0;
                fseek(fp, local_offset, SEEK_SET);
                size_t to_read = part_size - local_offset;
                if (to_read > size - read_total) to_read = size - read_total;
                fread(buf + read_total, 1, to_read, fp);
                read_total += to_read;
...
static int baymax_flush(const char *path, struct fuse_file_info *fi) {
    const char *filename = path + 1;
    if (strcmp(filename, FILENAME) != 0) return 0;
...
static int baymax_unlink(const char *path) {
    ...
    if (strcmp(filename, FILENAME) == 0) {
        ...
        if (deleted_count > 0) log_activity("DELETE", deleted_parts, "");
        return 0;
    }

//tambahan: untuk hapus file biasa
    char full_path[1024];
    snprintf(full_path, sizeof(full_path), "%s/%s", base_dir, filename);
